...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    float movementSpeed = 2.0f;   // Speed for moving the logic space
    float zoomSpeed = 2.0f;       // Speed for zooming in/out

    // Move the logic window with W, A, S, D (up, left, down, right)
    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y += movementSpeed * deltaTime;   // Move up
    }
    if (window->KeyHold(GLFW_KEY_S)) {
        logicSpace.y -= movementSpeed * deltaTime;   // Move down
    }
    if (window->KeyHold(GLFW_KEY_A)) {
        logicSpace.x -= movementSpeed * deltaTime;   // Move left
    }
    if (window->KeyHold(GLFW_KEY_D)) {
        logicSpace.x += movementSpeed * deltaTime;   // Move right
    }

    // Zoom in and zoom out with Z and X
    if (window->KeyHold(GLFW_KEY_Z)) {
        logicSpace.width -= zoomSpeed * deltaTime;   // Zoom in (decrease width)
        logicSpace.height -= zoomSpeed * deltaTime;  // Zoom in (decrease height)
    }
    if (window->KeyHold(GLFW_KEY_X)) {
        logicSpace.width += zoomSpeed * deltaTime;   // Zoom out (increase width)
        logicSpace.height += zoomSpeed * deltaTime;  // Zoom out (increase height)
    }
