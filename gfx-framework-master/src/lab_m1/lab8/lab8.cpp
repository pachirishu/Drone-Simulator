...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        materialKs = 0.5;
		useSpotlight = false;
		secondLightPosition = glm::vec3(2, 2, 2);
		secondLightDirection = glm::normalize(glm::vec3(-1, -1, 0));
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int light_position = glGetUniformLocation(shader->program, "light_position[0]");
    glUniform3f(light_position, lightPosition.x, lightPosition.y, lightPosition.z);

	int second_light_position = glGetUniformLocation(shader->program, "light_position[1]");
	glUniform3f(second_light_position, secondLightPosition.x, secondLightPosition.y, secondLightPosition.z);

    int light_direction = glGetUniformLocation(shader->program, "light_direction[0]");
    glUniform3f(light_direction, lightDirection.x, lightDirection.y, lightDirection.z);

	int second_light_direction = glGetUniformLocation(shader->program, "light_direction[1]");
    ...
    float cutOffAngle = glm::radians(15.0f);
	int loc_cut_off = glGetUniformLocation(shader->program, "cutOffAngle");
	glUniform1f(loc_cut_off, cutOffAngle);

	int loc_use_spotlight = glGetUniformLocation(shader->program, "useSpotlight");
	glUniform1i(loc_use_spotlight, useSpotlight ? 1 : 0);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
		...
		if (window->KeyHold(GLFW_KEY_I)) secondLightPosition -= forward * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_J)) secondLightPosition -= right * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_K)) secondLightPosition += forward * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_L)) secondLightPosition += right * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_O)) secondLightPosition += up * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_U)) secondLightPosition -= up * deltaTime * speed;
...
void Lab8::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_F)
	{
        ...
        cout << (useSpotlight ? "Spotlight mode enabled" : "Normal lighting mode enabled") << endl;
	}
