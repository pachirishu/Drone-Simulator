...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-1.5f, 0.75f, 1.5f));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (isPerspective) {
        float fovChangeSpeed = 1.0f * deltaTime;
        ...
        if (window->KeyHold(GLFW_KEY_MINUS)) {
            ...
            fov = glm::max(fov, RADIANS(30.0f));
            projectionMatrix = glm::perspective(fov, window->props.aspectRatio, perspectiveNear, perspectiveFar);
        }
    }

    float orthoChangeSpeed = 2.0f * deltaTime;

    if (!isPerspective) {
   
        if (window->KeyHold(GLFW_KEY_J)) {
            orthoLeft -= orthoChangeSpeed;
            orthoRight += orthoChangeSpeed;
            projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, orthoNear, orthoFar);
        }
        ...
        if (window->KeyHold(GLFW_KEY_I)) {
            orthoBottom -= orthoChangeSpeed;
            orthoTop += orthoChangeSpeed;
            projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, orthoNear, orthoFar);
        }
