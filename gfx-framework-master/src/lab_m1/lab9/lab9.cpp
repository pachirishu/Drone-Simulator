...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, mapTextures["earth"], mapTextures["crate"], 1);
    }
...

    {
        ...
        glm::vec3 direction = glm::normalize(glm::vec3(cameraPosition.x - quadPosition.x, 0.0f, cameraPosition.z - quadPosition.z));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, int mode)
{
    ...
    if (texture1)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, texture1->GetTextureID()); // Bind texture1 ID
        GLint loc_texture1 = glGetUniformLocation(shader->program, "texture_1"); // Get uniform location
        glUniform1i(loc_texture1, 0); // Send the uniform value (texture unit 0)
    ...
    if (texture2)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, texture2->GetTextureID()); // Bind texture2 ID
        GLint loc_texture2 = glGetUniformLocation(shader->program, "texture_2"); // Get uniform location
        glUniform1i(loc_texture2, 1); // Send the uniform value (texture unit 1)

    }

    GLint loc_mode = glGetUniformLocation(shader->program, "mode");
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    // Setare parametri texturã - Filtrare
