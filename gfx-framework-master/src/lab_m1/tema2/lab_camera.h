...
namespace implemented
{
    class CameraTema
    {
     public:
        ...
        void UpdateCamera(const glm::vec3& dronePosition, const glm::vec3& droneForward, const glm::vec3& droneUp, float distanceBehind, float heightAbove)
        {
            // Setam noua pozitie a camerei
            position = dronePosition - droneForward * distanceBehind + droneUp * heightAbove;

            // Camera trebuie sa priveasca catre drona
            forward = glm::normalize(dronePosition - position);

            // Recalculam vectorii up si right ai camerei
            right = glm::normalize(glm::cross(forward, droneUp));
        ...
        void RotateFirstPerson_OX(float angle)
        {
            ...
            forward = glm::normalize(glm::rotate(glm::mat4(1.0f), angle, right) * glm::vec4(forward, 0.0f));
            up = glm::normalize(glm::cross(right, forward));
        ...
        void RotateFirstPerson_OY(float angle)
        {
            ...
            // vectors!
            forward = glm::normalize(glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 1, 0)) * glm::vec4(forward, 0.0f));
            right = glm::normalize(glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 1, 0)) * glm::vec4(right, 0.0f)) ;
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            right = glm::normalize(glm::rotate(glm::mat4(1.0f), angle, forward) * glm::vec4(right, 0.0f));
            up = glm::normalize(glm::cross(right, forward));
